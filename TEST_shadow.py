...
while True:
    ...
    white_pixels = cv.countNonZero(adaptive_thresh)


    
    if white_pixels > 2900 and prev_left_fit is not None and prev_right_fit is not None:

        ploty = np.linspace(0, adaptive_thresh.shape[0]-1, adaptive_thresh.shape[0])
        ...
        center_fit = np.polyfit(ploty, center_fitx, 2)
        a, b, c = center_fit  
        lane_overlay = np.dstack((adaptive_thresh, adaptive_thresh, adaptive_thresh))  
    else:

...
while True:
    ...
    #foreground = cv.subtract(gray, background)
    foreground = cv.addWeighted(gray, 1.5, background, -0.5, 0)
    foreground = cv.normalize(foreground, None, 0, 255, cv.NORM_MINMAX)
    cv.imshow("forground" , foreground)
